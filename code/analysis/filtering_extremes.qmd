...
process_crop <- function(crop) {
  ...
  nc_list <- lapply(nc_vars, function(v) {
    nc_open(file.path(base_path, paste0("GGCMI-validation/data/processed/extremes_indicators/", v, "_", crop, ".nc")))
  })
  ...
  inner_join(extremes_filtered %>%  dplyr::select(difftrend_obs, divtrend_obs_perc, divtrend_obs, aggregated_yield_obs, climate_extreme, lat, lon, year, model) %>%  distinct(), by = c("lat", "lon", "year", "model"))
...
Loop over all crops, execute the function and store each crop type as RDS file.
...
for (crop in crops) {
  ...
  # Define output paths
  rds_path <- file.path(base_path, paste0("GGCMI-validation/data/processed/extremes_", crop, ".RDS"))
