...
base_map <- ggplot() +
  ...
  theme(
    ...
    plot.margin = unit(c(0.2, 0.2, 0.2, 0.2), "cm")  # Increase overall margins
  )
...
# Save the figure
ggsave(file.path(base_path, "GGCMI-validation/results/main/fig1a_GlobalPerformanceMap.png"), map_rsquared, width = 18, height = 8, dpi = 300)
```
...
lat_plot <- ggplot(ens_map_aggr, aes(x = lat, y = rsquared)) +
  ...
  coord_flip() +
  xlim(-45, 60) + # Limit latitude range between -45 and 60
  ylim(0, 1)
...
ggsave(file.path(base_path, "GGCMI-validation/results/main/fig1b_LatitudinalScatter.png"),
     lat_plot, width = 10, height = 8, dpi = 300)
```
